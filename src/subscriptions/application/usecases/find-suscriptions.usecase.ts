...
export class FindSubscriptionsUseCase {
    ...
    async run(): Promise<ISubscription[]> {
        const foundSubscription: ISubscription[] = await this.dataServices.subscriptions.find({});
        return foundSubscription.map((gym) => this.subscriptionsService.serializeSubscription(gym));
    }
}